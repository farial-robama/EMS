...
const DashboardLayout = ({ children }) => {
  ...
  const navRef = useRef(null);
  const savedScrollRef = useRef(0); // persists scroll position across navigations
  ...
  // ── Restore sidebar scroll after navigation or menu toggle ────────────────
  // requestAnimationFrame ensures the DOM has finished expanding/collapsing
  // before we set scrollTop, so there is no visible jump.
  useEffect(() => {
    const nav = navRef.current;
    if (!nav) return;
