...
=======
  // Apply dark mode class to document elements on mount and when darkMode changes
  useEffect(() => {
    if (darkMode) {
      document.documentElement.classList.add('dark');
      document.body.classList.add('dark');
  ...
  const handleToggleDarkMode = () => {
    toggleTheme(); // Use the toggleTheme from the hook
>>>>>>> ca9beb7 (Add user groups, role management, user profile and login page update)
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 theme-transition">
      {/* ================= SIDEBAR ================= */}
      <aside
        className={`fixed top-0 left-0 h-full bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 transition-all duration-300 theme-transition z-40 ${
          sidebarCollapsed ? 'w-20' : 'w-64'
        } ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}
      >
        {/* Header */}
      ...
      <div className={`transition-all duration-300 ${sidebarCollapsed ? 'lg:ml-20' : 'lg:ml-64'}`}>
        {/* Header */}
        <header className="sticky top-0 z-30 h-16 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm theme-transition transition-colors duration-300 ease-in-out">
          <div className="h-full px-4 sm:px-6 lg:px-8 flex items-center justify-between">
            {/* Left */}
            <div className="flex items-center gap-4">
              <button
        ...
        {/* Main */}
        <main className="min-h-[calc(100vh-8rem)] p-4 sm:p-6 lg:p-8 bg-gray-50 dark:bg-gray-900 theme-transition transition-colors duration-300 ease-in-out">{children}</main>

        {/* Footer */}
        <footer className="bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 py-4 theme-transition transition-colors duration-300 ease-in-out">
          <div className="px-4 sm:px-6 lg:px-8">
            <p className="text-center text-sm text-gray-600 dark:text-gray-400">
              © {new Date().getFullYear()}{' '}
              <span className="font-semibold">Advance IT Solutions</span> - EMS Admin Panel. All rights reserved.
