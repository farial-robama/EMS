...
// ─── Normalize any role string to a consistent lowercase_underscore key ───────
// Examples:
...
export const normalizeRole = (role) => {
  if (!role) return '';
  return role
    // Insert underscore before uppercase letters that follow lowercase (camelCase split)
...
export const AuthProvider = ({ children }) => {
  ...
  const [token, setToken]                 = useState(null);
  const [isAuthenticated, setIsAuthenticated] = useState(false);
  const [isLoading, setIsLoading]         = useState(true); // true until checkAuth finishes
  ...
  const login = async (userId, password) => {
    try {
      ...
      // Normalize the role so the rest of the app always gets a consistent string
      ...
      return { user: normalizedUser, token: authToken, success: true };
    } catch (error) {
      console.error('❌ AuthContext Login Error:', error);
      throw error;
    }
  ...
  const refreshUserData = async () => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/user/${user._id}`, {
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
      });
      if (!res.ok) throw new Error('Failed to fetch user data');
      const result      = await res.json();
      const updatedUser = { ...result.data, role: normalizeRole(result.data?.role) };
      setUser(updatedUser);
      localStorage.setItem('user_data', JSON.stringify(updatedUser));
      return updatedUser;
    } catch (error) {
  ...
  const checkAuth = () => {
    try {
      ...
      if (storedToken && storedUserData) {
        const userData = JSON.parse(storedUserData);
        // Ensure previously stored data also has a normalized role
      ...
      setIsLoading(false); // ← always unblock the app
    }
  };
