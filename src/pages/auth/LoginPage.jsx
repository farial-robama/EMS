...
const DEMO_CREDENTIALS = { userId: 'ADM0629', password: 'ADM0629' };
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .ldemo-header:hover { background: rgba(99,102,241,0.08); }
  .ldemo-header.open { border-bottom-color: rgba(99,102,241,0.15); }
  ...
  .ldemo-chevron.open { transform: rotate(180deg); }
  ...
  .ldemo-body.open { max-height: 120px; }
  ...
  .ldemo-btn:active { transform: translateY(0); }
  .ldemo-btn:disabled { opacity: 0.4; cursor: not-allowed; transform: none; }
...
const DemoLogin = ({ onFill, disabled }) => {
  const [open, setOpen] = useState(false);

  return (
    <div className="ldemo">
      <div
        className={`ldemo-header${open ? ' open' : ''}`}
        onClick={() => setOpen((v) => !v)}
      >
        <span className="ldemo-label">
          <span className="ldemo-dot" />
          Demo Access
        </span>
        <span className={`ldemo-chevron${open ? ' open' : ''}`}>▼</span>
      </div>
      <div className={`ldemo-body${open ? ' open' : ''}`}>
        <div className="ldemo-cred">
          <div className="ldemo-info">
            <div className="ldemo-row">
              <span className="ldemo-key">User ID</span>
              <span className="ldemo-val">{DEMO_CREDENTIALS.userId}</span>
            </div>
            <div className="ldemo-row">
              <span className="ldemo-key">Password</span>
              <span className="ldemo-val">{DEMO_CREDENTIALS.password}</span>
            </div>
          </div>
          <button
            type="button"
...
const LoginPage = () => {
  ...
  const handleDemoFill = ({ userId, password }) => {
    ...
    <div className="lr">
      ...
      <div className="lc">
        <div className="lc-inner">
          ...
          <DemoLogin onFill={handleDemoFill} disabled={isSubmitting} />
