...
import {
  Building2, Plus, Search, Pencil, X, Check,
...
const INITIAL = [
  { id: 1, name: 'Magura Collectorate Collegiate School', code: 'magura-collectorate-collegiate-school', status: 'Active',   reportDate: '2025-01-10' },
  { id: 2, name: 'Dhaka Model College',                  code: 'dhaka-model-college',                  status: 'Active',   reportDate: '2025-02-01' },
  { id: 3, name: 'Chittagong City Campus',               code: 'chittagong-city-campus',               status: 'Inactive', reportDate: '2025-03-15' },
];

const EMPTY = { name: '', status: 'Active', reportDate: new Date().toISOString().slice(0, 10) };
...
export default function CampusSetup() {
  ...
  const filtered = useMemo(() =>
    campuses.filter(c => c.name.toLowerCase().includes(search.toLowerCase())),
    [campuses, search]
  );
  ...
  const openAdd  = () => { setErrors({}); setModal({ mode: 'add',  data: { ...EMPTY } }); };
  const openEdit = (c) => { setErrors({}); setModal({ mode: 'edit', data: { ...c } }); };
  ...
  const handleSave = () => {
    ...
    if (!window.confirm(modal.mode === 'add' ? 'Add this campus?' : 'Update this campus?')) return;

    if (modal.mode === 'add') {
      setCampuses(p => [...p, { ...modal.data, id: Date.now(), code: slugify(modal.data.name) }]);
    } else {
      setCampuses(p => p.map(c => c.id === modal.data.id
    ...
    <DashboardLayout>
      <div className="space-y-5">
        ...
        <nav className="flex items-center gap-1.5 text-xs text-gray-400 dark:text-gray-500">
          {['Dashboard', 'Global Configurations', 'Institute Setup', 'Campus Setup'].map((item, i, arr) => (
            <React.Fragment key={item}>
              <span className={i === arr.length - 1 ? 'text-gray-700 dark:text-gray-200 font-medium' : 'hover:text-blue-500 cursor-pointer transition-colors'}>
                {item}
              </span>
              {i < arr.length - 1 && <ChevronRight size={12} className="text-gray-300 dark:text-gray-600" />}
            </React.Fragment>
          ))}
        </nav>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            { label: 'Total Campuses', value: totalCount,    color: 'bg-blue-50 dark:bg-blue-900/20',   icon: 'bg-blue-100 dark:bg-blue-800 text-blue-600 dark:text-blue-300' },
            { label: 'Active',         value: activeCount,   color: 'bg-green-50 dark:bg-green-900/20', icon: 'bg-green-100 dark:bg-green-800 text-green-600 dark:text-green-300' },
            { label: 'Inactive',       value: inactiveCount, color: 'bg-red-50 dark:bg-red-900/20',     icon: 'bg-red-100 dark:bg-red-800 text-red-600 dark:text-red-300' },
          ].map(s => (
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-100 dark:border-gray-700">
                  {['#', 'Campus Name', 'Campus Code', 'Report Date', 'Status', 'Action'].map(h => (
                ...
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-5 py-16 text-center">
                      ...
                      <p className="text-sm text-gray-400 dark:text-gray-500">
                        {search ? 'No campuses match your search.' : 'No campuses added yet.'}
                      </p>
                    </td>
                  </tr>
                ) : (
                  filtered.map((c, i) => (
                    <tr key={c.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                      ...
                      <td className="px-5 py-3.5 text-sm font-medium text-gray-800 dark:text-gray-100">{c.name}</td>
                      ...
                      <td className="px-5 py-3.5">
                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold
                          ...
                          <span className={`w-1.5 h-1.5 rounded-full ${c.status === 'Active' ? 'bg-green-500' : 'bg-red-400'}`} />
                          {c.status}
                        </span>
                      </td>
                      <td className="px-5 py-3.5">
