...
export default function AccountClosing() {
  ...
  const handleSave = () => {
    ...
    if (accounts.find((a) => a.month === label)) {
      showToast('This month is already added.');
      return;
    }
  ...
  const handleClose = (id) => {
    setAccounts((prev) =>
      prev.map((a) => (a.id === id ? { ...a, closed: true } : a))
    );
  ...
  const handleExportCSV = () => {
    ...
    accounts.forEach(
      (a) =>
        (csv += `${a.month},${a.previousBalance},${a.totalIncome},${a.totalExpense},${net(a)}\n`)
    );
    const blob = new Blob([csv], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = 'account_closing.csv';
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            <div
              key={label}
              className={`p-4 rounded-xl border ${bg} flex items-center gap-3`}
            >
              <div
              ...
              <div>
                <p className={`text-lg font-bold leading-tight ${vc}`}>
                  {fmt(val)}
                </p>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 bg-gray-50 dark:bg-gray-700/30 border-b border-gray-100 dark:border-gray-700">
            ...
            <span className="text-xs bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 px-2 py-0.5 rounded-full font-medium">
              {accounts.filter((a) => a.closed).length} closed ·{' '}
              {accounts.filter((a) => !a.closed).length} open
            </span>
          </div>
            ...
            <table className="w-full">
              ...
              <tbody>
                {accounts.map((acc) => (
                  ...
                  >
                    <td className="px-5 py-3 text-sm font-medium text-gray-700 dark:text-gray-200 whitespace-nowrap">
                      {acc.month}
                    </td>
                    <td className="px-5 py-3 text-sm text-gray-600 dark:text-gray-300">
                      {fmt(acc.previousBalance)}
                    </td>
                    ...
                    <td className="px-5 py-3 text-sm font-bold text-gray-800 dark:text-gray-100">
                      {fmt(net(acc))}
                    </td>
                ...
                <tr className="bg-gray-50 dark:bg-gray-700/40 border-t-2 border-gray-200 dark:border-gray-600 font-semibold">
                  <td className="px-5 py-3 text-sm font-bold text-gray-700 dark:text-gray-200">
                    Totals
                  </td>
                  <td className="px-5 py-3 text-sm text-gray-700 dark:text-gray-200">
                    {fmt(totals.prev)}
                  </td>
                  <td className="px-5 py-3 text-sm text-green-600 dark:text-green-400">
                    {fmt(totals.inc)}
                  </td>
                  <td className="px-5 py-3 text-sm text-red-500">
                    {fmt(totals.exp)}
                  </td>
                  <td className="px-5 py-3 text-sm font-bold text-purple-700 dark:text-purple-400">
                    {fmt(totals.net)}
                  </td>
                  <td colSpan={2} />
                </tr>
