...
import {
  ChevronRight, Plus, Search, FileText, Layout, Settings,
...
const INITIAL_TEMPLATES = [
  { id: 1, campus: "Mohammadpur Kendriya College", shift: "Day", medium: "Bangla", className: "HSC-Science",     eduLevel: "Higher Secondary", department: "Science",     session: "2025-2026" },
  { id: 2, campus: "Mohammadpur Kendriya College", shift: "Day", medium: "Bangla", className: "HSC-Humanities",  eduLevel: "Higher Secondary", department: "Humanities",  session: "2025-2026" },
  { id: 3, campus: "Mohammadpur Kendriya College", shift: "Day", medium: "Bangla", className: "HSC-Business",    eduLevel: "Higher Secondary", department: "Business",    session: "2025-2026" },
  { id: 4, campus: "Mohammadpur Kendriya College", shift: "Day", medium: "English", className: "HSC-Science",    eduLevel: "Higher Secondary", department: "Science",     session: "2025-2026" },
  { id: 5, campus: "Mohammadpur Kendriya College", shift: "Evening", medium: "Bangla", className: "HSC-Arts",   eduLevel: "Higher Secondary", department: "Arts",       session: "2024-2025" },
];
...
export default function FeeCollectionTemplate() {
  ...
  const handleSessionChange = (id, value) =>
    setTemplates(prev => prev.map(t => t.id === id ? { ...t, session: value } : t));
  ...
  const deptColor = (dept) => {
    const m = { Science: "bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400", Arts: "bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400", Humanities: "bg-pink-100 dark:bg-pink-900/30 text-pink-700 dark:text-pink-400", Business: "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400", Commerce: "bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400" };
    return m[dept] || "bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400";
  };
    ...
    <DashboardLayout>
      <div className="space-y-5">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <Breadcrumb items={["Dashboard", "Accounts", "Fee Collection Templates"]} />
          <button onClick={() => navigate("/feeCollectionTemplate/create")}
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "Active Sessions", val: [...new Set(templates.map(t => t.session))].length,        color: "bg-green-50 dark:bg-green-900/20 border-green-100 dark:border-green-800",  vc: "text-green-700 dark:text-green-400" },
            { label: "Departments",     val: [...new Set(templates.map(t => t.department))].length,     color: "bg-purple-50 dark:bg-purple-900/20 border-purple-100 dark:border-purple-800", vc: "text-purple-700 dark:text-purple-400" },
          ].map(s => (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
          ...
          <div className="flex items-center justify-between gap-3 px-5 py-4 bg-gray-50 dark:bg-gray-700/30 border-b border-gray-100 dark:border-gray-700 flex-wrap gap-y-3">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">Fee Collection Template List</span>
            </div>
            <div className="flex items-center gap-3 flex-wrap">
              <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                ...
                <select value={perPage} onChange={e => { setPerPage(+e.target.value); setPage(1); }}
                  ...
                  {PER_PAGES.map(n => <option key={n} value={n}>{n}</option>)}
                </select>
                entries
              </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-700/30 border-b border-gray-100 dark:border-gray-700">
                  {["SL","Campus","Shift","Medium","Class","Edu. Level","Department","Session","Action"].map(h => (
              ...
              <tbody>
                {current.length === 0 ? (
                  <tr>
                    <td colSpan={9} className="px-5 py-12 text-center text-sm text-gray-400 dark:text-gray-500">No templates found</td>
                  </tr>
                ) : (
                  current.map((t, i) => (
                    <tr key={t.id}
                      ...
                      <td className="px-4 py-3 text-xs text-gray-600 dark:text-gray-300 max-w-[140px] truncate" title={t.campus}>{t.campus}</td>
                      ...
                      <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{t.medium}</td>
                      <td className="px-4 py-3 text-sm font-medium text-gray-700 dark:text-gray-200 whitespace-nowrap">{t.className}</td>
                      ...
                      <td className="px-4 py-3">
                        <select value={t.session} onChange={e => handleSessionChange(t.id, e.target.value)}
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5">
                          <button onClick={() => navigate("/feeCollectionTemplate/template")}
                          ...
                          <button onClick={() => navigate("/feeCollectionTemplate/allocations")}
