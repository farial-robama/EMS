...
const COPY_COLORS = [
  ...
  {
    bg: 'bg-purple-50 dark:bg-purple-900/10 border-purple-100 dark:border-purple-800',
...
export default function InvoiceConfiguration() {
  ...
  const addSignatory = (id) =>
    setConfigs((prev) =>
      prev.map((c) =>
        c.id === id ? { ...c, signatories: [...c.signatories, ''] } : c
      )
    );
  ...
  const removeSignatory = (id, idx) =>
    setConfigs((prev) =>
      prev.map((c) =>
        c.id === id
          ? { ...c, signatories: c.signatories.filter((_, i) => i !== idx) }
          : c
      )
    );
    ...
    setConfigs((prev) =>
      prev.map((c) =>
        c.id === id
          ? {
              ...c,
              signatories: c.signatories.map((s, i) => (i === idx ? value : s)),
            }
          : c
      )
    );
    ...
    <DashboardLayout>
      <div className="space-y-5">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <Breadcrumb
            items={['Dashboard', 'Accounts', 'Invoice Configuration']}
          />
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
          ...
          <div className="px-5 py-3 text-xs text-gray-500 dark:text-gray-400">
            Configure each invoice copy with its name, description, footer
            notes, and the list of authorised signatories.
          </div>
        </div>
          ...
          {configs.map((config, ci) => {
            ...
            return (
              ...
              >
                {/* Card header */}
                <div
                  className={`flex items-center justify-between px-5 py-4 border-b border-inherit ${theme.header}`}
                >
                  <div className="flex items-center gap-2.5">
                    <div
                      className={`w-7 h-7 rounded-lg flex items-center justify-center ${theme.ic}`}
                    >
                      <FileText size={14} />
                    </div>
                    <input
                      ...
                      onChange={(e) =>
                        update(config.id, 'copyName', e.target.value)
                      }
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="flex flex-col gap-1.5">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          update(config.id, 'description', e.target.value)
                        }
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <input
                        ...
                        value={config.notes}
                        onChange={(e) =>
                          update(config.id, 'notes', e.target.value)
                        }
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button
                        onClick={() => addSignatory(config.id)}
                      ...
                      {config.signatories.map((sign, idx) => (
                        <div key={idx} className="flex items-center gap-2">
                          ...
                          <input
                            ...
                            value={sign}
                            onChange={(e) =>
                              updateSignatory(config.id, idx, e.target.value)
                            }
                            placeholder="Signatory name / role"
                            className={inp}
                          />
                          <button
                            onClick={() => removeSignatory(config.id, idx)}
