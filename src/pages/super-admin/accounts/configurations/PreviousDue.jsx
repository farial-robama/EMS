...
export default function PreviousDue() {
  ...
  const handleSearch = () => {
    setFiltered(applyFilter(students));
    setSearched(true);
  };
  ...
  const handlePay = (studentId, txIdx) => {
    const updated = students.map((s) =>
      s.id === studentId
        ? {
            ...s,
            transactions: s.transactions.map((t, i) =>
              i === txIdx ? { ...t, paid: true } : t
            ),
          }
        : s
    );
  ...
  const totalDue = (student) =>
    student.transactions.reduce((a, t) => a + (t.paid ? 0 : t.payable), 0);
  const initials = (name) =>
    name
      .split(' ')
      .map((w) => w[0])
      ...
      <div className="space-y-5">
        ...
        {!searched && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm p-12 text-center">
            ...
            <p className="text-sm text-gray-400 dark:text-gray-500">
              Use the search above to find students with previous dues.
            </p>
          </div>
        )}
              ...
              <div className="flex items-center gap-3">
                <div className="text-right">
                  ...
                  <p className="text-sm font-bold text-red-500">
                    {fmt(totalDue(student))}
                  </p>
                </div>
                <button
                  onClick={() => openAddDue(student)}
                  ...
                  {student.transactions.map((t, idx) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        {!t.paid && (
                          <button
                            onClick={() => handlePay(student.id, idx)}
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-xl p-6 max-w-sm w-full">
              ...
              <div className="space-y-4">
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                    Due Transaction Head
                  </label>
                  <select
                    value={newDue.head}
                    onChange={(e) =>
                      setNewDue((d) => ({ ...d, head: e.target.value }))
                    }
                    className={selCls}
                  >
                    <option value="">Select</option>
                    <option value="Tuition Fees (Due)">
                      Tuition Fees (Due)
                    </option>
                  ...
                  {dueErrors.head && (
                    <p className="text-xs text-red-500">{dueErrors.head}</p>
                  )}
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                  ...
                  <input
                    ...
                    value={newDue.month}
                    onChange={(e) =>
                      setNewDue((d) => ({ ...d, month: e.target.value }))
                    }
                    className={inp}
                  />
                  {dueErrors.month && (
                    <p className="text-xs text-red-500">{dueErrors.month}</p>
                  )}
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                  ...
                  <input
                    ...
                    value={newDue.amount}
                    onChange={(e) =>
                      setNewDue((d) => ({ ...d, amount: e.target.value }))
                    }
                    min={0}
                    className={inp}
                  />
                  {dueErrors.amount && (
                    <p className="text-xs text-red-500">{dueErrors.amount}</p>
                  )}
                </div>
              </div>
