...
import {
  ChevronRight, Plus, Eye, Edit2, Trash2, X, Save, ToggleLeft, ToggleRight, Tag,
} from "lucide-react";
...
export default function TransactionTypes() {
  ...
  const openView = (t) => { setMode("view"); setForm(t);    setErrors({}); setShowForm(true); };
  const openEdit = (t) => { setMode("edit"); setForm(t);    setErrors({}); setShowForm(true); };
    ...
    <DashboardLayout>
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: "Total Types", val: types.length,                                         color: "bg-blue-50 dark:bg-blue-900/20 border-blue-100 dark:border-blue-800",     vc: "text-blue-700 dark:text-blue-400" },
            { label: "Active",      val: types.filter(t => t.status === "Active").length,      color: "bg-green-50 dark:bg-green-900/20 border-green-100 dark:border-green-800",  vc: "text-green-700 dark:text-green-400" },
            { label: "Inactive",    val: types.filter(t => t.status === "Inactive").length,    color: "bg-gray-50 dark:bg-gray-700/30 border-gray-200 dark:border-gray-600",      vc: "text-gray-600 dark:text-gray-400" },
          ].map(s => (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
          <div className="flex items-center gap-2.5 px-5 py-4 bg-gray-50 dark:bg-gray-700/30 border-b border-gray-100 dark:border-gray-700">
            ...
            <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">Transaction Types</span>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-700/30 border-b border-gray-100 dark:border-gray-700">
                  {["#","Transaction Type Title","Status","Action"].map(h => (
              ...
              <tbody>
                {types.length === 0 ? (
                  <tr><td colSpan={4} className="px-5 py-12 text-center text-sm text-gray-400 dark:text-gray-500">No transaction types found</td></tr>
                ) : types.map((t, i) => (
                  <tr key={t.id}
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-xl p-6 max-w-sm w-full">
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-base font-semibold text-gray-800 dark:text-gray-100">
                  {mode === "add" ? "Add" : mode === "edit" ? "Edit" : "View"} Transaction Type
                </h3>
              ...
              <div className="space-y-4">
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                    Transaction Type Title <span className="text-red-500">*</span>
                  </label>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <select value={form.status} disabled={mode === "view"}
                    onChange={e => setForm(f => ({ ...f, status: e.target.value }))} className={selCls}>
        ...
        {deleteId && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
            <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-xl p-6 max-w-sm w-full">
              ...
              <h3 className="text-base font-semibold text-gray-800 dark:text-gray-100 text-center mb-1">Delete Transaction Type?</h3>
              <p className="text-sm text-gray-400 dark:text-gray-500 text-center mb-5">
                "<span className="font-medium text-gray-700 dark:text-gray-200">{types.find(t => t.id === deleteId)?.title}</span>" will be permanently removed.
              </p>
