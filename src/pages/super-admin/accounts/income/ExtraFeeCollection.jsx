...
export default function ExtraFeeCollection() {
  ...
  const handleSearch = () => {
    if (!studentID && !studentRoll)
      return alert('Please enter Student ID or Roll');
    setIsSearched(true);
  };

  const handleGenerateInvoice = () =>
    navigate('/extraFeeCollection/invoice', {
      state: { student: STUDENT, entries, totalAmount, cause },
    });

  const InfoRow = ({ label, val }) => (
    <div className="flex items-start gap-2">
      <span className="text-xs text-gray-400 dark:text-gray-500 w-28 flex-shrink-0">
        {label}
      </span>
  ...
  return (
    <DashboardLayout>
      <div className="space-y-5">
        <Breadcrumb
          items={['Dashboard', 'Accounts', 'Income', 'Extra Fee Collection']}
        />
              ...
              <div className="p-5">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end mb-5">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    >
                      <Check size={15} />{' '}
                      {markedPaid ? 'Marked as Paid' : 'Mark As Paid'}
                    </button>
                  </div>
                </div>
