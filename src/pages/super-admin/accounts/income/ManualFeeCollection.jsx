...
const MONTHLY_FEES = [
  { month: "July-2023", formFillup: 3450, notPromoted: 2000, subtotal: 4000, totalWaiver: 5450, totalPayable: 5450, totalPaid: 5450 },
];
...
export default function ManualFeeCollection() {
  ...
  const handlePayment = () => navigate("/manualFeeCollection/makePayment", {
  ...
  return (
    <DashboardLayout>
      <div className="space-y-5">
        <Breadcrumb items={["Dashboard", "Accounts", "Income", "Manual Fee Collection"]} />
          ...
          <>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
              <div className="flex items-center gap-2.5 px-5 py-4 bg-gray-50 dark:bg-gray-700/30 border-b border-gray-100 dark:border-gray-700">
                ...
                <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">Fee Collection Template</span>
              </div>
              <div className="p-5">
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 items-end">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input type="text" value={fmt(totalAmount)} readOnly className={inp + " bg-gray-100 dark:bg-gray-600 cursor-default"} />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Total Waiver</label>
                    <input type="text" value={fmt(totalWaiver)} readOnly className={inp + " bg-gray-100 dark:bg-gray-600 cursor-default"} />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Payable Amount</label>
                    <input type="text" value={fmt(payableAmount)} readOnly className={inp + " bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 font-semibold cursor-default border-green-200 dark:border-green-700"} />
                  </div>
                </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-gray-50 dark:bg-gray-700/30 border-b border-gray-100 dark:border-gray-700">
                      {["SL","Month","Form Fill-up","Not Promoted Fee","Sub Total","Total Waiver","Total Payable","Total Paid"].map(h => (
                  ...
                  <tbody>
                    {MONTHLY_FEES.map((fee, i) => (
                      <tr key={i} className="border-b border-gray-50 dark:border-gray-700/50 last:border-b-0 hover:bg-gray-50 dark:hover:bg-gray-700/20 transition-colors">
                        ...
                        <td className="px-4 py-3 text-sm font-medium text-gray-700 dark:text-gray-200">{fee.month}</td>
                        <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{fmt(fee.formFillup)}</td>
                        <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{fmt(fee.notPromoted)}</td>
                        <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{fmt(fee.subtotal)}</td>
                        <td className="px-4 py-3 text-sm text-amber-600 dark:text-amber-400">{fmt(fee.totalWaiver)}</td>
                        <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-200 font-medium">{fmt(fee.totalPayable)}</td>
                        <td className="px-4 py-3 text-sm text-green-600 dark:text-green-400 font-medium">{fmt(fee.totalPaid)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
