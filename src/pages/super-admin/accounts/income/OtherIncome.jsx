...
const TX_HEADS = ["Donation", "Service", "Miscellaneous", "Grant", "Sponsorship"];
...
export default function OtherIncome() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    navigate("/otherIncome/invoice", { state: { entries, invoiceDate, month, year, checkNo, checkDate, file, description, totalAmount } });
  };

  return (
    <DashboardLayout>
      <div className="space-y-5">
        <Breadcrumb items={["Dashboard", "Accounts", "Income", "Other Income"]} />

        {/* Summary strip */}
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          <div className="p-4 rounded-xl border bg-blue-50 dark:bg-blue-900/20 border-blue-100 dark:border-blue-800">
            <p className="text-2xl font-bold text-blue-700 dark:text-blue-400">{entries.length}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Transaction Entries</p>
          </div>
          <div className="p-4 rounded-xl border bg-green-50 dark:bg-green-900/20 border-green-100 dark:border-green-800">
            <p className="text-2xl font-bold text-green-700 dark:text-green-400">{fmt(totalAmount)}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Total Amount</p>
          </div>
          <div className="p-4 rounded-xl border bg-purple-50 dark:bg-purple-900/20 border-purple-100 dark:border-purple-800">
            <p className="text-2xl font-bold text-purple-700 dark:text-purple-400">{entries.filter(e => e.transactionHead).length}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Filled Entries</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 bg-gray-50 dark:bg-gray-700/30 border-b border-gray-100 dark:border-gray-700">
              <div className="flex items-center gap-2.5">
                <div className="w-7 h-7 rounded-lg flex items-center justify-center bg-blue-100 dark:bg-blue-800 text-blue-600 dark:text-blue-300"><Plus size={14} /></div>
                <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">Transaction Entries</span>
            ...
            <div className="p-5 space-y-3">
              ...
              <div className="flex justify-end pt-2 border-t border-gray-100 dark:border-gray-700">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-base font-bold text-green-600 dark:text-green-400">{fmt(totalAmount)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
            <div className="flex items-center gap-2.5 px-5 py-4 bg-gray-50 dark:bg-gray-700/30 border-b border-gray-100 dark:border-gray-700">
              <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">Invoice Details</span>
            </div>
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Invoice Date</label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Check No</label>
                  <input type="text" value={checkNo} onChange={e => setCheckNo(e.target.value)} placeholder="Enter check number" className={inp} />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Check Date</label>
                  <input type="date" value={checkDate} onChange={e => setCheckDate(e.target.value)} className={inp} />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Attachment</label>
                  ...
                  <button type="button" onClick={() => fileRef.current?.click()}
                    ...
                    <span className="text-gray-500 dark:text-gray-400 truncate">{file ? file.name : "Choose file…"}</span>
                  </button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
                <div className="sm:col-span-2 flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Purpose / Description</label>
                  <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Enter purpose or description" rows={3}
