...
import {
  ChevronRight, Download, Upload, FileSpreadsheet, X, Check,
  AlertCircle, CloudUpload, SlidersHorizontal, FileX, Eye,
} from 'lucide-react';
...
const TEMPLATE_COLUMNS = [
  'Student Name','Roll No','Student Code','Gender','Religion',
  'Father Name','Mother Name','Contact No','Shift','Medium',
  'Edu Level','Department','Class',
];
...
const StepBadge = ({ num, label, active, done }) => (
  <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border transition-all
    ...
    <span className={`text-sm font-medium ${active ? 'text-blue-700 dark:text-blue-400' : done ? 'text-green-700 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'}`}>{label}</span>
  </div>
);

/* ── Main Component ──────────────────────────────────────────────────────── */
export default function StudentBulkUpload() {
  const [filters, setFilters] = useState({ shift: '', medium: '', eduLevel: '', department: '', className: '' });
  ...
  const handleDownload = () => {
    ...
    const rows = Array.from({ length: 10 }, () => ({
      'Student Name': '', 'Roll No': '', 'Student Code': '', 'Gender': '', 'Religion': '',
      'Father Name': '', 'Mother Name': '', 'Contact No': '',
      Shift: filters.shift, Medium: filters.medium, 'Edu Level': filters.eduLevel,
      Department: filters.department, Class: filters.className,
    }));

    const ws = XLSX.utils.json_to_sheet(rows);
    ws['!cols'] = TEMPLATE_COLUMNS.map(h => ({ wch: Math.max(h.length + 4, 18) }));
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Students');
    XLSX.writeFile(wb, `StudentTemplate_${filters.className}_${filters.eduLevel}.xlsx`);
  };

  /* File selection */
  const processFile = useCallback(f => {
    if (!f) return;
    if (!f.name.match(/\.(xlsx|xls)$/i)) { setUploadError('Please upload a valid .xlsx or .xls file.'); return; }
    setFile(f);
    setUploadError('');
    setUploadDone(false);
    ...
    reader.onload = evt => {
      ...
      } catch { setUploadError('Could not parse Excel file.'); setFile(null); }
    };
  ...
  const clearFile = () => { setFile(null); setPreview(null); setUploadError(''); setUploadDone(false); if (fileRef.current) fileRef.current.value = ''; };
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          ...
          <h1 className="text-xl font-bold text-gray-800 dark:text-white">Student Bulk Upload</h1>
        </div>
      </div>

      {/* Step indicators */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <StepBadge num={1} label="Select Filters & Download Template" active={step === 1} done={step > 1} />
        <StepBadge num={2} label="Fill Template & Choose File"        active={step === 2} done={step > 2} />
        <StepBadge num={3} label="Preview & Upload"                   active={step === 3} done={uploadDone} />
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
        <div className="flex items-center gap-2.5 px-5 py-4 bg-gray-50 dark:bg-gray-700/30 border-b border-gray-100 dark:border-gray-700">
          ...
          <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">Step 1 — Generate Excel Template</span>
          {allFiltersSelected && <Check size={14} className="ml-auto text-green-500" />}
        </div>
        <div className="p-5 space-y-4">
          <p className="text-xs text-gray-500 dark:text-gray-400">Select all filters to generate a pre-filled Excel template for bulk student data entry.</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            {[
              ...
              <div key={f.name} className="flex flex-col gap-1.5">
                ...
                {filterErrors[f.name] && (
                  <p className="text-xs text-red-500 flex items-center gap-0.5"><AlertCircle size={10} />Required</p>
                )}
              </div>
            ))}
          </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
        <div className="flex items-center gap-2.5 px-5 py-4 bg-gray-50 dark:bg-gray-700/30 border-b border-gray-100 dark:border-gray-700">
          <div className="w-7 h-7 rounded-lg bg-green-100 dark:bg-green-900 flex items-center justify-center">
            <CloudUpload size={14} className="text-green-600 dark:text-green-400" />
          </div>
          <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">Step 2 & 3 — Upload Filled Excel</span>
        </div>
        <div className="p-5 space-y-4">
          {/* Drop zone */}
          {!file && (
            <div
              ...
              <div className="text-center">
                <p className="text-sm font-semibold text-gray-700 dark:text-gray-200">Drag & drop your Excel file here</p>
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">or click to browse — .xlsx, .xls only</p>
              </div>
          ...
          {file && (
            <div className="flex items-center gap-4 p-4 rounded-xl border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900/10">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-green-600 dark:text-green-400 mt-0.5">{(file.size / 1024).toFixed(1)} KB · {preview?.length || 0} data rows detected</p>
              </div>
              <button onClick={clearFile} className="w-8 h-8 rounded-lg bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 hover:bg-red-50 hover:text-red-500 text-gray-500 flex items-center justify-center transition-all flex-shrink-0">
                <X size={13} />
              </button>
            </div>
          )}
          ...
          {uploadDone && (
            <div className="flex items-center gap-3 p-4 rounded-xl border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900/10">
              ...
              <div>
                ...
                <p className="text-xs text-green-600 dark:text-green-500 mt-0.5">Students have been queued for processing.</p>
              </div>
            </div>
          )}
          ...
          {preview && preview.length > 0 && (
            <div className="rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
              <div className="flex items-center gap-2 px-4 py-3 bg-gray-50 dark:bg-gray-700/30 border-b border-gray-100 dark:border-gray-700">
                ...
                <span className="text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wide">Preview (first {preview.length} rows)</span>
              </div>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-100 dark:border-gray-700">
                      ...
                      {Object.keys(preview[0]).length > 8 && <th className="px-3 py-2 text-gray-400">+{Object.keys(preview[0]).length - 8} more</th>}
                    </tr>
                  </thead>
                    ...
                    {preview.map((row, i) => (
                      <tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-700/20">
                        {Object.values(row).slice(0, 8).map((v, j) => (
                          <td key={j} className="px-3 py-2 text-gray-700 dark:text-gray-300 whitespace-nowrap">{String(v) || <span className="text-gray-300 dark:text-gray-600 italic">—</span>}</td>
                        ))}
                      </tr>
                    ))}
                  </tbody>
          ...
          {file && (
            <div className="flex items-center gap-3">
              ...
              <p className="text-xs text-gray-400 dark:text-gray-500">
                {preview ? `${preview.length} rows will be imported` : 'Preparing preview…'}
              </p>
            </div>
          )}
        </div>
      ...
      <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800 rounded-2xl p-5">
        <p className="text-sm font-semibold text-amber-800 dark:text-amber-400 mb-2">📋 Instructions</p>
        <ul className="text-xs text-amber-700 dark:text-amber-500 space-y-1">
          <li>1. Select all 5 filters above, then download the Excel template.</li>
          <li>2. Fill in student data in the downloaded template — do not change column headers or pre-filled metadata columns.</li>
          <li>3. Upload the completed file above. A preview will appear before you confirm.</li>
          <li>4. Only .xlsx and .xls formats are supported. Maximum 500 students per file.</li>
        </ul>
      </div>
    </div>
  );
