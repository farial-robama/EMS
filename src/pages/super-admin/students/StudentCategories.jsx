...
import {
  ChevronRight, Plus, Pencil, Trash2, Search, Tag, Check,
  AlertCircle, X, ChevronLeft, ChevronDown,
} from 'lucide-react';
...
const SEED = [
  ...
  { id: 3, title: 'Transfer Student', categoryCode: 'TS-003', status: 'Inactive' },
];
...
export default function StudentCategories() {
  ...
  const handleSave = async () => {
    ...
    if (modal === 'add') {
      ...
      setCategories(p => p.map(c => c.id === modal.id ? { ...c, ...form } : c));
    }
    closeModal();
  };
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-1">
          <Breadcrumb items={['Dashboard', 'Student Setup', 'Student Categories']} />
          <h1 className="text-xl font-bold text-gray-800 dark:text-white">Student Categories</h1>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: 'Total',    value: categories.length,                      bg: 'bg-blue-50 dark:bg-blue-900/20',   ic: 'bg-blue-100 dark:bg-blue-800 text-blue-600 dark:text-blue-300' },
          { label: 'Active',   value: categories.filter(c=>c.status==='Active').length,   bg: 'bg-green-50 dark:bg-green-900/20', ic: 'bg-green-100 dark:bg-green-800 text-green-600 dark:text-green-300' },
          { label: 'Inactive', value: categories.filter(c=>c.status!=='Active').length, bg: 'bg-red-50 dark:bg-red-900/20',     ic: 'bg-red-100 dark:bg-red-800 text-red-600 dark:text-red-300' },
        ].map(s => (
          <div key={s.label} className={`flex items-center gap-4 p-4 rounded-xl border border-gray-100 dark:border-gray-700 ${s.bg}`}>
            ...
            <div>
              ...
              <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{s.label} Categories</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-4 border-b border-gray-100 dark:border-gray-700">
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="Search title or code…" value={search} onChange={e => { setSearch(e.target.value); setPage(1); }}
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-100 dark:border-gray-700">
                {['#', 'Title', 'Category Code', 'Status', 'Actions'].map(h => (
                  <th key={h} className={`px-5 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide ${h === 'Actions' ? 'text-right' : ''}`}>{h}</th>
                ))}
              </tr>
            </thead>
              ...
              {paged.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-5 py-14 text-center">
                    ...
                    <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Try a different search or add a new category.</p>
                  </td>
                </tr>
              ) : paged.map((cat, i) => (
                <tr key={cat.id} className="hover:bg-gray-50/70 dark:hover:bg-gray-700/20 transition-colors">
                  <td className="px-5 py-4 text-sm text-gray-400 dark:text-gray-500">{(safePage - 1) * perPage + i + 1}</td>
                  <td className="px-5 py-4 text-sm font-semibold text-gray-800 dark:text-gray-100">{cat.title}</td>
                  <td className="px-5 py-4">
                    <span className="font-mono text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-2.5 py-1 rounded-lg">{cat.categoryCode}</span>
                  </td>
            ...
            <div className="p-5 space-y-4">
              <div className="flex flex-col gap-1.5">
                ...
                <input name="title" value={form.title} onChange={handleFormChange}
                  placeholder="e.g. New Student" className={errors.title ? inpErr : inp} />
                {errors.title && <p className="flex items-center gap-1 text-xs text-red-500"><AlertCircle size={11} />{errors.title}</p>}
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                  Category Code<span className="text-red-500 ml-0.5">*</span>
                </label>
                <input name="categoryCode" value={form.categoryCode} onChange={handleFormChange}
                  placeholder="e.g. NS-001" className={errors.categoryCode ? inpErr : inp} />
                {errors.categoryCode && <p className="flex items-center gap-1 text-xs text-red-500"><AlertCircle size={11} />{errors.categoryCode}</p>}
              </div>
              <div className="flex flex-col gap-1.5">
            ...
            <div className="flex items-center justify-end gap-2 px-5 py-4 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/30">
              ...
              <button onClick={handleSave} disabled={saving}
                ...
                {saving
                  ...
                  : <><Check size={13} />{modal === 'add' ? 'Save Category' : 'Update Category'}</>}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setDeleteConfirm(null)}>
          ...
          <div className="relative w-full max-w-sm bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 p-6 text-center"
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-5">
              <span className="font-semibold text-gray-700 dark:text-gray-200">"{deleteConfirm.title}"</span> will be permanently removed.
            </p>
