...
const DUMMY_STUDENTS = [
  {
    ...
    department: 'Science',
    address: 'House 12, Mohammadpur, Dhaka-1207',
  ...
  },
  {
    ...
    motherName: 'Sumaiya Khatun',
    className: 'HSC-Science',
    section: '1st Year',
    session: '2025-2026',
    eduLevel: 'Higher Secondary',
    department: 'Science',
    address: 'Road 5, Block B, Mirpur-1, Dhaka',
    subjectCode: '101,102,104',
    bloodGroup: 'A+',
    dob: '2007-07-22',
    studentId: 'STU240002',
  },
  {
    ...
    eduLevel: 'Higher Secondary',
    department: 'Science',
    address: 'Flat 4C, Banani, Dhaka-1213',
    subjectCode: '101,102,105',
    bloodGroup: 'O+',
    dob: '2006-11-08',
    studentId: 'STU240003',
  },
...
function IDCard({ student, showBack, showSubjectCode, showAddress }) {
  return (
    <div className="flex flex-col sm:flex-row gap-3">
      {/* Front */}
      <div
        className="w-full sm:w-72 bg-gradient-to-br from-blue-700 to-blue-900 rounded-2xl overflow-hidden shadow-lg text-white flex-shrink-0"
      ...
      >
        ...
        <div className="px-4 pt-4 pb-3 flex items-center gap-2 border-b border-blue-600/50">
          ...
          <div>
            ...
            <p className="text-[9px] text-blue-300 leading-tight mt-0.5">
        ...
        <div className="px-4 py-3 flex gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="grid grid-cols-2 gap-x-2 mt-2">
              <div>
                <p className="text-[8px] text-blue-400 uppercase tracking-wide">
                  Roll
                </p>
        ...
        <div className="px-4 py-2 border-t border-blue-600/50 bg-blue-900/50">
          <p className="text-[9px] text-blue-300">
            Father: <span className="text-white">{student.fatherName}</span>
          </p>
        </div>
      </div>
      ...
      {showBack && (
        <div
          className="w-full sm:w-72 bg-white dark:bg-gray-700 rounded-2xl border-2 border-dashed border-gray-200 dark:border-gray-600 overflow-hidden shadow-sm flex-shrink-0 flex flex-col"
          style={{ minHeight: '186px' }}
        >
...
export default function StudentIDCard() {
  ...
  const validate = () => {
    const errs = {};
    ['eduLevel', 'department', 'className', 'section', 'session'].forEach(
      (k) => {
  ...
  const handleShow = () => {
    ...
    const result = DUMMY_STUDENTS.filter(
      (s) =>
        (!filters.studentName ||
      ...
      <div className="space-y-5">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1">
            <Breadcrumb
              items={['Dashboard', 'Student Setup', 'Student ID Card']}
            />
              ...
              {FILTER_FIELDS.map((f) => (
                <div key={f.name} className="flex flex-col gap-1.5">
                  ...
                  <select
                    ...
                    className={
                      filterErrors[f.name] ? `${inp} border-red-400` : inp
                    }
                  >
            ...
            <div className="flex items-center justify-between">
              ...
              {students.length > 0 && (
                <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                  {filters.backPartType === 'Yes' && (
                    <span className="px-2 py-1 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-lg border border-blue-100 dark:border-blue-800">
                      Back side
                    </span>
                  )}
                  {filters.subjectCodeType === 'Yes' && (
                    <span className="px-2 py-1 bg-purple-50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400 rounded-lg border border-purple-100 dark:border-purple-800">
                      Subject codes
                    </span>
                  )}
                  {filters.addressType === 'Yes' && (
                    <span className="px-2 py-1 bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 rounded-lg border border-green-100 dark:border-green-800">
                      Address
                    </span>
                  )}
                </div>
            ...
            {students.length === 0 ? (
              <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 py-14 text-center">
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
                  Try adjusting the search criteria above.
                </p>
              </div>
            ) : (
              <div ref={cardRef} className="space-y-5">
                {students.map((s) => (
                  <div
                    key={s.roll}
                    className="bg-white dark:bg-gray-800/40 rounded-2xl border border-gray-100 dark:border-gray-700 p-5"
                  >
                    <IDCard
                      student={s}
