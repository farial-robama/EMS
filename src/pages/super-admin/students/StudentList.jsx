...
import {
  ChevronRight, Search, Pencil, Eye, Plus, Zap, Download,
  Users, ChevronLeft, SlidersHorizontal, X, FileText,
} from 'lucide-react';
...
const STUDENT_DATA = Array.from({ length: 50 }, (_, i) => ({
  ...
  formData: {
    fullNameBangla:'নাম', birthRegNo:'1234567890', dob:'2008-01-01',
    nationality:'Bangladeshi', country:'Bangladesh', religion:'Islam',
    bloodGroup:'A+', gender:'Male', maritalStatus:'Unmarried',
    email:`student${i+1}@email.com`,
    fatherName:'Father Name', fatherPhone:'01711111111',
    motherName:'Mother Name', motherPhone:'01722222222',
    guardianName:'Guardian Name', guardianPhone:'01733333333',
    roll: 70 + i, session: i % 2 === 0 ? '2024-2025' : '2025',
    hostel:'Magura Hostel', hostelRoom:'101', hostelType:'Boys',
    hostelFee:5000, paymentStatus:'Paid',
    busNo:'Bus 12', roadNo:'Road 5', pickupPoint:'Stop A', dropPoint:'Stop B',
    transportType:'Bus', driverName:'Driver Name', driverContact:'01755555555',
    transportFee:3000, transportPaymentStatus:'Paid',
  },
}));
  ...
  <nav className="flex items-center gap-1.5 text-xs text-gray-400 dark:text-gray-500">
    {items.map((item, i) => (
      <React.Fragment key={item}>
        <span className={i === items.length - 1 ? 'text-gray-700 dark:text-gray-200 font-semibold' : 'hover:text-blue-500 cursor-pointer transition-colors'}>{item}</span>
...
async function generatePDF(student) {
  ...
  doc.text('Student Detailed Profile', W / 2, y, { align: 'center' }); y += 10;
  doc.setFontSize(10).setTextColor('#64748B');
  doc.text(`Generated: ${new Date().toLocaleDateString()}`, W / 2, y + 10, { align: 'center' }); y += 30;

  // Avatar
  try { doc.addImage(student.image, 'JPEG', W - 110, 30, 70, 70); } catch {}

  const header = (title) => {
    if (y > 700) { doc.addPage(); y = 40; }
    doc.setFillColor('#DBEAFE').setDrawColor('#2563EB');
    doc.rect(20, y, W - 40, 22, 'FD');
    doc.setFontSize(13).setTextColor('#1D4ED8').text(title, 26, y + 16);
    y += 32;
  };
  ...
  autoTable(doc, {
    ...
    theme: 'grid',
    headStyles: { fillColor: '#DBEAFE', textColor: '#1D4ED8', fontStyle: 'bold' },
    styles: { fontSize: 10 },
  ...
  autoTable(doc, {
    ...
    body: [[formData.hostel, formData.hostelRoom, formData.hostelFee, formData.busNo, formData.transportFee]],
    theme: 'grid',
    headStyles: { fillColor: '#DBEAFE', textColor: '#1D4ED8', fontStyle: 'bold' },
    styles: { fontSize: 10 },
    didDrawPage: d => { y = d.cursor.y + 20; },
  ...
  // QR Code
  try {
    const qr = await QRCode.toDataURL(`https://ems.system/student/${student.studentId}`);
    if (y + 90 > 820) { doc.addPage(); y = 40; }
    doc.addImage(qr, 'PNG', W - 110, y, 80, 80);
    doc.setFontSize(8).setTextColor('#9CA3AF').text('Scan to verify', W - 110 + 40, y + 88, { align: 'center' });
  } catch {}

  doc.setFontSize(9).setTextColor('#9CA3AF').text('Generated by EMS System', W / 2, 820, { align: 'center' });
...
export default function StudentList() {
  ...
  const [filters, setFilters] = useState({ eduLevel:'', department:'', className:'', section:'', session:'', shift:'', medium:'' });
  ...
  const clearFilters = () => { setFilters({ eduLevel:'', department:'', className:'', section:'', session:'', shift:'', medium:'' }); setSearch(''); setPage(1); };
  ...
  const filtered = useMemo(() =>
    students.filter(s =>
      ...
      (!search || s.name.toLowerCase().includes(search.toLowerCase()) ||
        s.studentId.includes(search) || s.roll.toString().includes(search))
    ),
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-1">
          <Breadcrumb items={['Dashboard', 'Student Setup', 'Student List']} />
          <h1 className="text-xl font-bold text-gray-800 dark:text-white">Student List</h1>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          <button onClick={() => navigate('/quickAddStudent')}
          ...
          <button onClick={() => navigate('/addSingleStudent')}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: 'Total Students', value: students.length,            bg: 'bg-blue-50 dark:bg-blue-900/20',   ic: 'bg-blue-100 dark:bg-blue-800 text-blue-600 dark:text-blue-300' },
          { label: 'Filtered',       value: filtered.length,            bg: 'bg-purple-50 dark:bg-purple-900/20', ic: 'bg-purple-100 dark:bg-purple-800 text-purple-600 dark:text-purple-300' },
          { label: 'This Page',      value: paged.length,               bg: 'bg-green-50 dark:bg-green-900/20', ic: 'bg-green-100 dark:bg-green-800 text-green-600 dark:text-green-300' },
          { label: 'Active Filters', value: activeFilterCount,           bg: activeFilterCount > 0 ? 'bg-orange-50 dark:bg-orange-900/20' : 'bg-gray-50 dark:bg-gray-700/30', ic: activeFilterCount > 0 ? 'bg-orange-100 dark:bg-orange-900 text-orange-600 dark:text-orange-400' : 'bg-gray-100 dark:bg-gray-700 text-gray-400' },
        ].map(s => (
          <div key={s.label} className={`flex items-center gap-3 p-4 rounded-xl border border-gray-100 dark:border-gray-700 ${s.bg}`}>
            <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${s.ic}`}><Users size={16} /></div>
            <div>
              <div className="text-xl font-bold text-gray-800 dark:text-white leading-none">{s.value}</div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
        <button onClick={() => setFiltersOpen(o => !o)}
          ...
          <div className="flex items-center gap-2">
            ...
            <ChevronRight size={16} className={`text-gray-400 transition-transform duration-200 ${filtersOpen ? 'rotate-90' : ''}`} />
          </div>
        </button>
        {filtersOpen && (
          <div className="p-5">
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3">
              {[
                ...
                <div key={f.name} className="flex flex-col gap-1.5">
                  ...
                  <select name={f.name} value={filters[f.name]} onChange={handleFilterChange} className={inp}>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-4 border-b border-gray-100 dark:border-gray-700">
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="Search name, ID, roll…" value={search} onChange={e => { setSearch(e.target.value); setPage(1); }}
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-100 dark:border-gray-700">
                {['#','Student ID','Reg. No','Photo','Name','Roll','Shift','Medium','Department','Class','Section','Session','Contact','Actions'].map(h => (
              ...
              {paged.length === 0 ? (
                <tr>
                  <td colSpan={14} className="px-5 py-14 text-center">
                    <Users size={40} className="mx-auto text-gray-200 dark:text-gray-600 mb-3" />
                    <p className="text-sm font-medium text-gray-500 dark:text-gray-400">No students match your criteria</p>
                    <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Try adjusting the filters or search term.</p>
                  </td>
                </tr>
              ) : paged.map((s, i) => (
                <tr key={s.id} className="hover:bg-gray-50/70 dark:hover:bg-gray-700/20 transition-colors">
                  <td className="px-4 py-3.5 text-sm text-gray-400 dark:text-gray-500">{(safePage - 1) * perPage + i + 1}</td>
                  <td className="px-4 py-3.5">
                    <span className="font-mono text-xs bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 px-2 py-1 rounded-lg">{s.studentId}</span>
                  </td>
                  <td className="px-4 py-3.5 text-xs text-gray-500 dark:text-gray-400 font-mono">{s.regNo}</td>
                  <td className="px-4 py-3.5">
                    <img src={s.image} alt={s.name} className="w-9 h-9 rounded-xl object-cover border-2 border-white dark:border-gray-600 shadow-sm" onError={e => { e.target.style.display = 'none'; }} />
                  </td>
                  <td className="px-4 py-3.5 text-sm font-semibold text-gray-800 dark:text-gray-100 whitespace-nowrap">{s.name}</td>
                  <td className="px-4 py-3.5 text-sm font-mono text-gray-700 dark:text-gray-300">{s.roll}</td>
                  <td className="px-4 py-3.5 text-sm text-gray-600 dark:text-gray-400">{s.shift}</td>
                  <td className="px-4 py-3.5 text-sm text-gray-600 dark:text-gray-400">{s.medium}</td>
                  <td className="px-4 py-3.5">
                    <span className="text-xs bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-400 px-2 py-1 rounded-lg">{s.department}</span>
                  </td>
                  <td className="px-4 py-3.5 text-sm text-gray-600 dark:text-gray-400">{s.className}</td>
                  ...
                  <td className="px-4 py-3.5">
                    <div className="flex items-center justify-end gap-1.5">
                      <button onClick={() => handleViewPDF(s)} disabled={pdfLoading === s.id}
                      ...
                      <button onClick={() => navigate('/addSingleStudent', { state: { student: s } })}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-4 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/30">
          <p className="text-xs text-gray-500 dark:text-gray-400">
            ...
            {activeFilterCount > 0 && <span className="ml-1 text-blue-500">(filtered from {students.length} total)</span>}
          </p>
          <div className="flex items-center gap-1">
            <button onClick={() => setPage(1)} disabled={safePage === 1}
              className="w-8 h-8 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-500 disabled:opacity-40 hover:bg-gray-100 dark:hover:bg-gray-600 flex items-center justify-center transition-all text-xs font-bold">«</button>
                ...
                : <button key={p} onClick={() => setPage(p)}
                    className={`w-8 h-8 rounded-lg border text-xs font-medium transition-all
                      ${safePage === p
                        ...
                        : 'border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600'}`}>{p}</button>
              )}
