...
import {
  ChevronRight, Shuffle, Eye, Check, X, AlertCircle,
  SlidersHorizontal, ArrowRight, Users,
} from 'lucide-react';
...
const BLANK_MIGRATE = { migrateShift:'Day', migrateMedium:'Bangla', migrateEduLevel:'', migrateDepartment:'', migrateClass:'', migrateSection:'', migrateSession:'' };
const BLANK_OPTIONS = { archive:false, date: new Date().toISOString().split('T')[0] };
...
export default function StudentMigration() {
  ...
  const handleMigrateClick = () => {
    ...
    if (Object.keys(errs).length) { setMigrateErrors(errs); return; }
    setMigrateErrors({});
  ...
  const confirmMigrate = async () => {
    ...
    setSuccessMsg(`✓ ${selected.length} student${selected.length !== 1 ? 's' : ''} migrated to ${migrateFilters.migrateClass} — ${migrateFilters.migrateSection} (${migrateFilters.migrateSession})`);
    setSelected([]);
  };
  ...
  const MIGRATE_FIELDS = [
    { label:'Shift',      name:'migrateShift',      opts:['Day','Morning','Evening'],                req:false },
    { label:'Medium',     name:'migrateMedium',     opts:['Bangla','English'],                       req:false },
    { label:'Edu Level',  name:'migrateEduLevel',   opts:['','Higher Secondary','Secondary'],        req:false },
    { label:'Department', name:'migrateDepartment', opts:['','Science','Arts','Commerce'],           req:false },
    { label:'Class',      name:'migrateClass',      opts:['','HSC-Science','HSC-Arts','HSC-Commerce'],req:true  },
    { label:'Section',    name:'migrateSection',    opts:['','2nd Year','3rd Year','A','B'],          req:true  },
    { label:'Session',    name:'migrateSession',    opts:['','2026-2027','2027-2028'],                req:true  },
  ];
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-1">
        <Breadcrumb items={['Dashboard', 'Student Setup', 'Student Migration']} />
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
        <div className="flex items-center gap-2.5 px-5 py-4 bg-purple-50 dark:bg-purple-900/10 border-b border-purple-100 dark:border-purple-900/30">
          ...
          <span className="text-sm font-semibold text-purple-700 dark:text-purple-400">Current Class</span>
          <span className="text-xs text-purple-400/70">— Source class for migration</span>
        </div>
      ...
      {showTable && (
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
          <div className="px-5 py-4 bg-gray-50 dark:bg-gray-700/30 border-b border-gray-100 dark:border-gray-700">
            <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">Migration Options</span>
          </div>
          <div className="p-5 flex flex-wrap items-center gap-5">
            <label className="flex items-center gap-2.5 cursor-pointer group">
              <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all flex-shrink-0
                ...
                {options.archive && <Check size={11} className="text-white" />}
                <input type="checkbox" name="archive" checked={options.archive} onChange={handleOptions} className="sr-only" />
              </div>
              <span className="text-sm text-gray-700 dark:text-gray-200">Cancel Admission After Migration</span>
            </label>
      ...
      {showTable && (
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
          <div className="flex items-center gap-2.5 px-5 py-4 bg-purple-50 dark:bg-purple-900/10 border-b border-purple-100 dark:border-purple-900/30">
            ...
            <span className="text-sm font-semibold text-purple-700 dark:text-purple-400">Migrate To</span>
            <span className="text-xs text-purple-400/70">— Select destination class</span>
          </div>
          <div className="p-5 space-y-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3">
              {MIGRATE_FIELDS.map(f => (
                <F key={f.name} label={f.label} required={f.req} error={migrateErrors[f.name]}>
                  <select name={f.name} value={migrateFilters[f.name]} onChange={handleMigrate}
            ...
            {successMsg && (
              <div className="flex items-center gap-3 p-4 rounded-xl border border-purple-200 dark:border-purple-800 bg-purple-50 dark:bg-purple-900/10">
                ...
                <p className="text-sm font-medium text-purple-700 dark:text-purple-400">{successMsg}</p>
              </div>
            )}

            <div className="flex items-center gap-3">
              <button onClick={handleMigrateClick}
                ...
                <Shuffle size={15} />
                Migrate {selected.length > 0 ? `${selected.length} ` : ''}Selected
              </button>
              {selected.length === 0 && showTable && (
                <p className="text-xs text-gray-400 dark:text-gray-500 flex items-center gap-1">
                  <AlertCircle size={11} /> Select at least one student to migrate
                </p>
              )}
            </div>
          </div>
      ...
      {/* ── Confirm Modal ────────────────────────────────────────────────── */}
      {confirmModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => !migrating && setConfirmModal(false)}>
          ...
          <div className="relative w-full max-w-md bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden"
            ...
            <div className="flex items-center gap-2.5 px-5 py-4 bg-purple-50 dark:bg-purple-900/10 border-b border-purple-100 dark:border-purple-900/30">
              <div className="w-7 h-7 rounded-lg bg-purple-100 dark:bg-purple-900 flex items-center justify-center">
                <Shuffle size={14} className="text-purple-600 dark:text-purple-400" />
              </div>
              <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">Confirm Migration</span>
            </div>
            <div className="p-5 space-y-4">
              <p className="text-sm text-gray-600 dark:text-gray-300">
                You are about to migrate <span className="font-bold text-gray-800 dark:text-white">{selected.length} student{selected.length !== 1 ? 's' : ''}</span> to:
              </p>
            ...
            <div className="flex items-center justify-end gap-2 px-5 py-4 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/30">
              <button onClick={() => setConfirmModal(false)} disabled={migrating}
              ...
              <button onClick={confirmMigrate} disabled={migrating}
                ...
                {migrating
                  ? <><span className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" />Migrating…</>
                  : <><Check size={13} />Confirm Migration</>}
              </button>
            </div>
          </div>
        </div>
