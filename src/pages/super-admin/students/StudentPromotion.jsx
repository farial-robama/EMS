...
export default function StudentPromotion() {
  ...
  const confirmPromote = async () => {
    ...
    setSuccessMsg(
      `✓ ${selected.length} student${selected.length !== 1 ? 's' : ''} promoted to ${promoteFilters.promoteClass} — ${promoteFilters.promoteSection} (${promoteFilters.promoteSession})`
    );
    setSelected([]);
  };
  ...
  const PROMOTE_FIELDS = [
    ...
    {
      label: 'Edu Level',
      name: 'promoteEduLevel',
      opts: ['', 'Higher Secondary', 'Secondary'],
      req: false,
    },
    {
      label: 'Department',
      name: 'promoteDepartment',
      opts: ['', 'Science', 'Arts', 'Commerce'],
      req: false,
    },
    {
    ...
    <DashboardLayout>
      <div className="space-y-5">
        ...
        <div className="space-y-1">
          <Breadcrumb
            items={['Dashboard', 'Student Setup', 'Student Promotion']}
          />
          <h1 className="text-xl font-bold text-gray-800 dark:text-white flex items-center gap-2">
            <ArrowUpCircle size={22} className="text-blue-500" /> Student
            Promotion
          </h1>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
          <div className="flex items-center gap-2.5 px-5 py-4 bg-blue-50 dark:bg-blue-900/10 border-b border-blue-100 dark:border-blue-900/30">
            ...
            <span className="text-xs text-blue-500/70 dark:text-blue-500/60">
              — Select from class to promote
            </span>
          </div>
        ...
        {showTable && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
            ...
            <div className="p-5 flex flex-wrap items-center gap-5">
              {[
                {
                  name: 'prevCourseAdvising',
                  label: 'Copy Previous Course Advising',
                },
        ...
        {showTable && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
            <div className="flex items-center gap-2.5 px-5 py-4 bg-green-50 dark:bg-green-900/10 border-b border-green-100 dark:border-green-900/30">
              ...
              <span className="text-xs text-green-500/70 dark:text-green-500/60">
            ...
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3">
                {PROMOTE_FIELDS.map((f) => (
                  <F
                    ...
                    label={f.label}
                    required={f.req}
                    error={promoteErrors[f.name]}
                  >
                    <select
                      name={f.name}
                      value={promoteFilters[f.name]}
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  ...
                  Promote {selected.length > 0 ? `${selected.length} ` : ''}
                  Selected
                </button>
        ...
        {confirmModal && (
          <div
            ...
            onClick={() => !promoting && setConfirmModal(false)}
          >
            ...
            >
              ...
              <div className="p-5 space-y-4">
                ...
                {(options.prevCourseAdvising ||
                  options.prevClassRoll ||
                  options.archive) && (
                  <div className="flex flex-wrap gap-2">
                    {options.prevCourseAdvising && (
                      <span className="text-xs bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-2.5 py-1 rounded-full">
                        With Course Advising
                      </span>
                    )}
                    {options.prevClassRoll && (
                      <span className="text-xs bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 px-2.5 py-1 rounded-full">
                        Keep Class Roll
                      </span>
                    )}
                    {options.archive && (
