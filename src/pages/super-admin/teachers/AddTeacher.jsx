...
import {
  ChevronRight, ChevronDown, Check, User, Phone, GraduationCap,
  Briefcase, Upload, AlertCircle, ImageIcon, Copy,
} from "lucide-react";
...
const INITIAL = {
  employeeId: "", eduLevel: "", department: "", fullNameEn: "", fullNameBn: "",
  fatherName: "", motherName: "", dob: "", gender: "", religion: "", bloodGroup: "",
  nationalId: "", birthPlace: "", responsibility: "", designation: "",
  joiningDate: "", batchId: "", indexNo: "", teacherPicture: null,
  presentAddress: "", permanentAddress: "", contactPhone: "", mobileNo: "", email: "",
  bcsBatch: "", degreeName: "", divisionGPA: "", obtainYear: "", boardUniversity: "",
  otherDegreeName: "", otherDivisionGPA: "", otherObtainYear: "", otherBoardUniversity: "",
  resume: null, employeeType: "Teacher", instituteName: "", position: "",
  experienceYear: "", officeAddress: "", attendanceId: "", timeSchedule: "", ordering: "",
};
...
export default function AddTeacher() {
  ...
  useEffect(() => {
    setFormData(p => ({ ...p, employeeId: `EMP${String(Date.now()).slice(-6)}` }));
  }, []);
  ...
  const validate = () => {
    const e = {};
    if (!formData.fullNameEn.trim()) e.fullNameEn = "Full name (English) is required";
  ...
  const handleSubmit = async (e) => {
    ...
    setFormData({ ...INITIAL, employeeId: `EMP${String(Date.now()).slice(-6)}` });
    setPreviews({});
  };
    ...
    <DashboardLayout>
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <SectionCard title="Personal Information" icon={User}
          iconColor="bg-blue-100 dark:bg-blue-800 text-blue-600 dark:text-blue-300" badge="Required">
          <div className="flex flex-col sm:flex-row gap-6">
            <div className="flex-shrink-0">
              <PhotoUpload name="teacherPicture" label="Staff Photo"
                preview={previews.teacherPicture} onChange={handleChange} />
            </div>
            <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              <F label="Employee ID">
                <div className="flex gap-2">
                  <input value={formData.employeeId} readOnly className={roInp} />
                  <button type="button" title="Copy ID"
                    onClick={() => navigator.clipboard?.writeText(formData.employeeId)}
              ...
              <F label="Full Name (English)" required error={errors.fullNameEn}>
                <input name="fullNameEn" value={formData.fullNameEn} onChange={handleChange}
                  placeholder="Full name in English" className={inp(errors.fullNameEn)} />
              </F>
              <F label="Full Name (Bangla)">
                <input name="fullNameBn" value={formData.fullNameBn} onChange={handleChange}
                  placeholder="বাংলায় নাম লিখুন" className={inp(false)} />
              </F>
              <F label="Date of Birth">
                <input type="date" name="dob" value={formData.dob} onChange={handleChange} className={inp(false)} />
              </F>
              <F label="Gender" required error={errors.gender}>
                {sel("gender", ["Male", "Female", "Other"], errors.gender)}
              </F>
              <F label="Religion">
                {sel("religion", ["Islam", "Hindu", "Christian", "Buddhist", "Other"], false)}
              </F>
              <F label="Blood Group">
                {sel("bloodGroup", ["A+", "A-", "B+", "B-", "O+", "O-", "AB+", "AB-"], false)}
              </F>
              <F label="National ID">
                <input name="nationalId" value={formData.nationalId} onChange={handleChange} className={inp(false)} />
              </F>
              <F label="Birth Place">
                <input name="birthPlace" value={formData.birthPlace} onChange={handleChange} className={inp(false)} />
              </F>
              <F label="Country">
              ...
              <F label="Father / Husband Name">
                <input name="fatherName" value={formData.fatherName} onChange={handleChange} className={inp(false)} />
              </F>
              <F label="Mother Name">
                <input name="motherName" value={formData.motherName} onChange={handleChange} className={inp(false)} />
              </F>
            </div>
          </div>
        </SectionCard>
        ...
        <SectionCard title="Professional Details" icon={Briefcase}
          ...
          badge="Required" defaultOpen={false}>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <F label="Employee Type">
              {sel("employeeType", ["Teacher", "Employee"], false)}
            </F>
            <F label="Educational Level">
              {sel("eduLevel", ["Higher Secondary", "Graduate", "Post-Graduate", "PhD"], false)}
            </F>
            <F label="Department" required error={errors.department}>
              {sel("department", ["Science", "Arts", "Commerce", "Engineering", "Humanities"], errors.department)}
            </F>
            <F label="Designation" required error={errors.designation}>
              <input name="designation" value={formData.designation} onChange={handleChange}
                placeholder="e.g. Senior Teacher" className={inp(errors.designation)} />
            </F>
            <F label="Responsibility">
              <input name="responsibility" value={formData.responsibility} onChange={handleChange} className={inp(false)} />
            </F>
            <F label="Joining Date">
              <input type="date" name="joiningDate" value={formData.joiningDate} onChange={handleChange} className={inp(false)} />
            </F>
            <F label="Batch ID">
              <input name="batchId" value={formData.batchId} onChange={handleChange} className={inp(false)} />
            </F>
            <F label="Index No.">
              <input name="indexNo" value={formData.indexNo} onChange={handleChange} className={inp(false)} />
            </F>
            <F label="Attendance Machine ID">
              <input name="attendanceId" value={formData.attendanceId} onChange={handleChange} className={inp(false)} />
            </F>
            <F label="Time Schedule">
              <input name="timeSchedule" value={formData.timeSchedule} onChange={handleChange}
                placeholder="e.g. 8:00 AM – 2:00 PM" className={inp(false)} />
            </F>
            <F label="Display Ordering">
              <input type="number" name="ordering" value={formData.ordering} onChange={handleChange} min={0} className={inp(false)} />
            </F>
          </div>
        </SectionCard>
        ...
        <SectionCard title="Contact Information" icon={Phone}
          iconColor="bg-green-100 dark:bg-green-800 text-green-600 dark:text-green-300" defaultOpen={false}>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <F label="Mobile No.">
              <input name="mobileNo" value={formData.mobileNo} onChange={handleChange}
                placeholder="+880 1X XX-XXXXXX" className={inp(false)} />
            </F>
            <F label="Phone">
              <input name="contactPhone" value={formData.contactPhone} onChange={handleChange} className={inp(false)} />
            </F>
            <F label="BCS Batch No.">
              <input name="bcsBatch" value={formData.bcsBatch} onChange={handleChange} className={inp(false)} />
            </F>
            <div className="col-span-full grid grid-cols-1 sm:grid-cols-2 gap-4">
              <F label="Present Address">
                <textarea name="presentAddress" value={formData.presentAddress} onChange={handleChange}
                  placeholder="House, Road, Area, City" className={taCls} />
              </F>
        ...
        <SectionCard title="Educational Qualification" icon={GraduationCap}
          iconColor="bg-orange-100 dark:bg-orange-800 text-orange-600 dark:text-orange-300" defaultOpen={false}>
          <div className="space-y-5">
            <div>
              <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">Primary Degree</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <F label="Degree Name">
                  <input name="degreeName" value={formData.degreeName} onChange={handleChange} className={inp(false)} />
                </F>
                <F label="Division / GPA">
                  <input name="divisionGPA" value={formData.divisionGPA} onChange={handleChange} className={inp(false)} />
                </F>
                <F label="Obtain Year">
                  <input name="obtainYear" value={formData.obtainYear} onChange={handleChange} className={inp(false)} />
                </F>
                <F label="Board / University">
                  <input name="boardUniversity" value={formData.boardUniversity} onChange={handleChange} className={inp(false)} />
                </F>
              </div>
            </div>
            <div className="border-t border-gray-100 dark:border-gray-700 pt-4">
              <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">Other Degree</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <F label="Degree Name">
                  <input name="otherDegreeName" value={formData.otherDegreeName} onChange={handleChange} className={inp(false)} />
                </F>
                <F label="Division / GPA">
                  <input name="otherDivisionGPA" value={formData.otherDivisionGPA} onChange={handleChange} className={inp(false)} />
                </F>
                <F label="Obtain Year">
                  <input name="otherObtainYear" value={formData.otherObtainYear} onChange={handleChange} className={inp(false)} />
                </F>
                <F label="Board / University">
                  <input name="otherBoardUniversity" value={formData.otherBoardUniversity} onChange={handleChange} className={inp(false)} />
                </F>
              </div>
            </div>
          </div>
        ...
        <SectionCard title="Work Experience & Documents" icon={Upload}
          iconColor="bg-teal-100 dark:bg-teal-800 text-teal-600 dark:text-teal-300" defaultOpen={false}>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <F label="Institute / Company Name">
              <input name="instituteName" value={formData.instituteName} onChange={handleChange} className={inp(false)} />
            </F>
            <F label="Position Held">
              <input name="position" value={formData.position} onChange={handleChange} className={inp(false)} />
            </F>
            <F label="Years of Experience">
              <input type="number" name="experienceYear" value={formData.experienceYear}
                onChange={handleChange} min={0} className={inp(false)} />
            </F>
            <F label="Office Address">
              <input name="officeAddress" value={formData.officeAddress} onChange={handleChange} className={inp(false)} />
            </F>
            <F label="Resume Upload (PDF only)">
              <input type="file" name="resume" accept=".pdf" onChange={handleChange} className={fileInp} />
              {formData.resume && (
