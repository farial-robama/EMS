...
import {
  ChevronRight, Upload, Check, AlertCircle, FileSpreadsheet, Download, X,
} from "lucide-react";
...
export default function BulkTeacherUpload() {
  ...
  const handleUpload = async (e) => {
    ...
    const iv = setInterval(() => {
      ...
      if (p >= 100) {
        clearInterval(iv);
        setProgress(100);
        setTimeout(() => { setUploading(false); setSuccess(true); }, 400);
      }
  ...
  const reset = () => { setFile(null); setSuccess(false); setProgress(0); };
      ...
      <div className="space-y-5">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
          <div className="flex items-center gap-2.5 px-5 py-4 bg-gray-50 dark:bg-gray-700/30 border-b border-gray-100 dark:border-gray-700">
            ...
            <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">Bulk Teacher Upload</span>
            <span className="text-xs bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-2 py-0.5 rounded-full font-medium">Excel</span>
          </div>
          <div className="p-5">
            {/* Steps */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
              {[
                { num: "1", title: "Download Template", desc: "Get the Excel template with the correct column structure." },
                { num: "2", title: "Fill in Data", desc: "Enter teacher details into the template without changing headers." },
                { num: "3", title: "Upload File", desc: "Upload the completed file to import all teachers at once." },
              ].map(s => (
                <div key={s.num} className="flex gap-3 p-3 rounded-xl bg-gray-50 dark:bg-gray-700/30 border border-gray-100 dark:border-gray-700">
                  <div className="w-7 h-7 rounded-lg bg-blue-600 text-white text-xs font-bold flex items-center justify-center flex-shrink-0">
                    {s.num}
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-gray-700 dark:text-gray-200 mb-0.5">{s.title}</p>
            ...
            {!success ? (
              <>
                ...
                <div className="flex items-start gap-2 mt-4 p-3 rounded-lg bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800">
                  ...
                  <p className="text-xs text-amber-700 dark:text-amber-400 leading-relaxed">
                    <strong>Do not modify column headers</strong> in the template. Ensure all required fields are filled before uploading.
                  </p>
                </div>
              ...
              <div className="text-center py-8">
                ...
                <h3 className="text-base font-semibold text-gray-800 dark:text-gray-100 mb-1">Upload Successful!</h3>
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-5">
                  <span className="font-medium text-gray-700 dark:text-gray-200">{file?.name}</span> has been imported and is being processed.
                </p>
