...
export default function DepartmentalHead() {
  ...
  const confirmEdit = (idx) => {
    setDepartments((prev) =>
      prev.map((d, i) => (i === idx ? { ...d, head: editVal } : d))
    );
    setEditingIdx(null);
    setEditVal('');
  };
    ...
    <DashboardLayout>
      <div className="space-y-5">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <Breadcrumb
            items={['Dashboard', 'Teacher Setup', 'Departmental Head']}
          />
          <button
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {[
            ...
            },
            {
              ...
              valColor: 'text-green-700 dark:text-green-400',
            },
            {
              label: 'Unassigned',
              val: departments.length - assignedCount,
              color:
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-700/30 border-b border-gray-100 dark:border-gray-700">
                  <th className="px-5 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide w-12">
                    SL
                  </th>
                  <th className="px-5 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                    Department Name
                  </th>
                  <th className="px-5 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                    Departmental Head
                  </th>
                  <th className="px-5 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide w-20">
                    Action
                  </th>
                </tr>
              </thead>
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  filtered.map((dept, i) => {
                    ...
                    const realIdx = departments.findIndex(
                      (d) => d.name === dept.name
                    );
