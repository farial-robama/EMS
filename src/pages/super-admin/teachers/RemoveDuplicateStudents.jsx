...
import {
  ChevronRight, Search, Trash2, Check, AlertCircle, RefreshCw, ShieldAlert,
} from "lucide-react";
...
const CRITERIA = [
  ...
  { id: "contact", label: "Same Contact Info",   desc: "Match by phone or email" },
];

const MOCK_DUPLICATES = [
  {
    name: "Rahim Uddin", initials: "RU",
    entries: [
  ...
  {
    name: "Karim Hossain", initials: "KH",
    entries: [
...
const TOTAL_DUPES   = MOCK_DUPLICATES.reduce((acc, g) => acc + g.entries.filter(e => !e.keep).length, 0);
...
export default function RemoveDuplicateStudents() {
  ...
  const handleReset = () => {
    setState("idle"); setProgress(0); setConfirmed(false);
  };
    ...
    <DashboardLayout>
      <div className="space-y-5">
        ...
        <div className="flex items-start justify-between gap-4">
          <Breadcrumb items={["Dashboard", "Student Setup", "Remove Duplicates"]} />
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
          ...
          <div className="flex items-center gap-2.5 px-5 py-4 bg-gray-50 dark:bg-gray-700/30 border-b border-gray-100 dark:border-gray-700">
            ...
            <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">Remove Duplicate Students</span>
            {state === "results" && (
              ...
              <>
                ...
                <div className="flex items-start gap-2.5 p-3 rounded-xl bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-800 mb-6">
                  ...
                  <div className="text-xs text-red-700 dark:text-red-400 leading-relaxed space-y-1">
                    ...
                    <p>• This tool scans all student records for duplicates based on selected criteria.</p>
                    <p>• You can review all matches before any deletion occurs.</p>
                    <p>• Deleted records <strong>cannot be recovered</strong> — proceed carefully.</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
                  {CRITERIA.map(c => {
                    ...
                    return (
                      <button key={c.id} type="button" onClick={() => toggleCriteria(c.id)}
                ...
                <div className="flex justify-end">
                  <button type="button" onClick={handleScan} disabled={!criteria.length}
            ...
            {state === "scanning" && (
              <div className="py-10 text-center">
                ...
                <p className="text-sm text-gray-400 dark:text-gray-500 mb-5">
                  Comparing {TOTAL_SCANNED} records using {criteria.length} {criteria.length === 1 ? "criterion" : "criteria"}
                </p>
            ...
            {state === "results" && (
              <>
                ...
                <div className="grid grid-cols-3 gap-3 mb-5">
                  {[
                    { label: "Records Scanned", val: TOTAL_SCANNED, color: "bg-gray-50 dark:bg-gray-700/30 border-gray-100 dark:border-gray-700", valColor: "text-gray-800 dark:text-gray-100" },
                    { label: "Duplicates Found", val: TOTAL_DUPES,   color: "bg-red-50 dark:bg-red-900/20 border-red-100 dark:border-red-800",    valColor: "text-red-600 dark:text-red-400" },
                    { label: "Unique Records",   val: TOTAL_SCANNED - TOTAL_DUPES, color: "bg-green-50 dark:bg-green-900/20 border-green-100 dark:border-green-800", valColor: "text-green-600 dark:text-green-400" },
                  ].map(s => (
                ...
                <div className="rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden mb-5">
                  <div className="px-4 py-2.5 bg-gray-50 dark:bg-gray-700/30 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
                    ...
                    <span className="text-xs text-gray-400 dark:text-gray-500">✓ = Original to keep</span>
                  </div>
                  {MOCK_DUPLICATES.map((g, gi) => (
                    <div key={gi} className="border-b border-gray-100 dark:border-gray-700 last:border-b-0">
                      {/* Group header */}
                      <div className="flex items-center gap-3 px-4 py-3 bg-red-50/50 dark:bg-red-900/10">
                        ...
                        <span className="text-sm font-semibold text-gray-700 dark:text-gray-200 flex-1">{g.name}</span>
                        <span className="text-xs bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 px-2 py-0.5 rounded-full font-medium">
                      ...
                      {g.entries.map((entry, ei) => (
                        <div key={ei}
                          ...
                          {entry.keep
                            ...
                            : <input type="checkbox" defaultChecked className="accent-red-500 flex-shrink-0" />}
                          <code className="text-gray-400 dark:text-gray-500 font-mono">{entry.id}</code>
                          <span className="flex-1 text-gray-600 dark:text-gray-300">{entry.detail}</span>
                          {entry.keep && (
                ...
                <label className="flex items-start gap-2.5 cursor-pointer mb-5">
                  ...
                  <span className="text-sm text-gray-600 dark:text-gray-300">
                    I understand this action is <strong>irreversible</strong> and will permanently delete {TOTAL_DUPES} duplicate records.
                  </span>
                </label>
                ...
                <div className="flex justify-between items-center gap-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button type="button" onClick={handleRemove} disabled={!confirmed}
                      className={`flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-white rounded-lg transition-colors shadow-sm
