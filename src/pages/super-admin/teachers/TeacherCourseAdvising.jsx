...
import { ChevronRight, ChevronDown, Search, Check, BookOpen, User, AlertCircle } from "lucide-react";
...
const TEACHERS = [
  {
    code: "E000216", name: "Md. Nazrul Islam", department: "Science",
    eduLevel: "Higher Secondary", session: "2025-2026", shift: "Day", medium: "Bangla",
    courses: [
      {
        className: "HSC-Science",
        sections: ["2nd Year", "1st Year", "1st Year (A)"],
        subjects: [
          "Bangla", "English", "Bangla 1st Part", "Bangla 2nd Part",
          "English 1st Part", "English 2nd Part", "Physics", "Physics 1st Part",
          "Physics 2nd Part", "Chemistry", "Chemistry 1st Part", "Chemistry 2nd Part",
          "Biology", "Biology 1st Paper", "Biology 2nd Part",
          "Higher Mathematics", "Higher Mathematics 1st Part", "Higher Mathematics 2nd Part", "ICT",
        ],
      },
      {
        ...
        subjects: [
          "Bangla 1st Part", "Bangla", "Bangla 2nd Part",
          "English 1st Part", "English", "English 2nd Part", "ICT",
          "Economics 1st Part", "Economics", "Economics 2nd Part",
          "Logic 1st Part", "Logic", "Logic 2nd Part",
          "Islamic History 1st Part", "Islamic History", "Islamic History 2nd Part",
          "Civics 1st Part", "Civics", "Civics 2nd Part",
          "Islamic Studies", "Islamic Studies 1st Part", "Islamic Studies 2nd Part",
          "Psychology", "Psychology 1st Part", "Psychology 2nd Part",
          "Geography", "Geography 1st Part", "Geography 2nd Part",
          "Social Work 1st Paper", "Social Work 2nd Paper",
          "Home Science 1st Part", "Home Science 2nd Part",
        ],
      },
    ],
  },
  {
    code: "E000217", name: "Marufa", department: "Arts",
    eduLevel: "Higher Secondary", session: "2025-2026", shift: "Day", medium: "English",
    courses: [
      {
        className: "HSC-Arts",
...
export default function TeacherCourseAdvising() {
  const [filters, setFilters] = useState({
    eduLevel: "", session: "", shift: "", medium: "", department: "", teacherCode: "",
  });
  ...
  const handleShow = () => {
    ...
    if (t) {
      const init = {};
      t.courses.forEach(c => c.sections.forEach(s => { init[`${c.className}||${s}`] = true; }));
  ...
  const toggleSubject = (className, section, subject, checked) => {
    ...
    setSelectedSubjects(prev => {
      const cur = prev[key] || [];
      return { ...prev, [key]: checked ? [...cur, subject] : cur.filter(s => s !== subject) };
    });
  };
    ...
    <DashboardLayout>
      <div className="space-y-5">
        ...
        <div className="flex items-start justify-between gap-4">
          <Breadcrumb items={["Dashboard", "Teacher Setup", "Course Advising"]} />
        </div>
        ...
        {/* Filters */}
        <SectionCard title="Filter Teachers" icon={Search}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 mb-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select name="shift" value={filters.shift} onChange={handleFilterChange} className={selCls}>
                <option value="">All</option>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select name="medium" value={filters.medium} onChange={handleFilterChange} className={selCls}>
                <option value="">All</option>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select name="teacherCode" value={filters.teacherCode} onChange={handleFilterChange} className={selCls}>
                <option value="">All</option>
                {TEACHERS.map(t => <option key={t.code} value={t.code}>{t.name}</option>)}
              </select>
            </div>
          </div>
        ...
        {selectedTeacher && (
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
            ...
            <SectionCard title="Class & Section Wise Subjects" icon={BookOpen}
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {[
                  ...
                  <span key={i.label} className="inline-flex items-center gap-1 text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-2.5 py-1 rounded-lg">
                    <span className="font-semibold text-gray-400">{i.label}:</span> {i.val}
                  </span>
                ))}
              </div>
              ...
              <div className="space-y-4">
                {selectedTeacher.courses.map((course, ci) => (
                  <div key={ci} className="rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                    {/* Class header */}
                    <div className="px-4 py-3 bg-blue-50 dark:bg-blue-900/20 border-b border-blue-100 dark:border-blue-800">
                      <p className="text-xs font-bold text-blue-700 dark:text-blue-400 uppercase tracking-wide">
                        {course.className} — {selectedTeacher.medium} Medium · {selectedTeacher.shift} Shift
                      </p>
                    </div>
                    ...
                    {course.sections.map((sec, si) => {
                      ...
                      return (
                        <div key={si} className="border-b border-gray-100 dark:border-gray-700 last:border-b-0">
                          {/* Section toggle */}
                          <button type="button" onClick={() => toggleSection(key)}
                            ...
                            <span className="text-xs font-semibold text-gray-700 dark:text-gray-200">{sec}</span>
                            <div className="flex items-center gap-2">
                              ...
                              <ChevronDown size={14} className={`text-gray-400 transition-transform ${openSections[key] ? "rotate-180" : ""}`} />
                            </div>
                          </button>
                                ...
                                return (
                                  <label key={j}
                                    ...
                                    <input type="checkbox" checked={checked}
                                      onChange={e => toggleSubject(course.className, sec, sub, e.target.checked)}
                                      className="hidden" />
                                    {sub}
                                  </label>
                                );
            ...
            <div className="flex flex-col gap-5">
              <SectionCard title="Allocated Course(s)" icon={User}
                iconColor="bg-green-100 dark:bg-green-800 text-green-600 dark:text-green-300"
                badge={allAllocated.length > 0 ? `${allAllocated.length} subjects` : undefined}>
                ...
                {allAllocated.length === 0 ? (
                  <div className="text-center py-10">
                    ...
                    <p className="text-sm text-gray-400 dark:text-gray-500">
                      No subjects selected yet.<br />Check subjects from the left panel.
                    </p>
                  </div>
                ) : (
                  <>
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-100 dark:border-gray-700">
                            {["Shift", "Medium", "Edu. Level", "Class", "Section", "Subject"].map(h => (
                        ...
                        <tbody>
                          {Object.entries(selectedSubjects).map(([key, subs]) => {
                            ...
                            return subs.map((sub, idx) => (
                              <tr key={`${key}-${sub}-${idx}`}
                                ...
                                <td className="px-3 py-2 text-xs text-gray-600 dark:text-gray-300">{selectedTeacher.shift}</td>
                                <td className="px-3 py-2 text-xs text-gray-600 dark:text-gray-300">{selectedTeacher.medium}</td>
                                <td className="px-3 py-2 text-xs text-gray-600 dark:text-gray-300">{selectedTeacher.eduLevel}</td>
                                <td className="px-3 py-2 text-xs text-gray-600 dark:text-gray-300 whitespace-nowrap">{className}</td>
                                <td className="px-3 py-2 text-xs text-gray-600 dark:text-gray-300 whitespace-nowrap">{section}</td>
                                <td className="px-3 py-2 text-xs font-medium text-gray-800 dark:text-gray-100">{sub}</td>
                              </tr>
                            ));
                          })}
                        </tbody>
        ...
        {selectedTeacher === null && filters.teacherCode !== "" && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm p-10 text-center">
            ...
            <p className="text-sm font-medium text-gray-600 dark:text-gray-300">No teacher found matching the selected filters.</p>
          </div>
        )}
