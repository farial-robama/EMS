...
import { ChevronRight, Download, Printer, User, IdCard, QrCode } from "lucide-react";
...
const TEACHERS = [
  {
    code: "E000001", name: "Kumud Ranjan Biswas", designation: "Assistant Teacher",
    dob: "1970-01-01", mobile: "01700000000", department: "Science",
    eduLevel: "Higher Secondary", shift: "Day",
    subjects: ["Bangla", "Physics", "Chemistry"], photo: "",
  },
  {
    code: "E000002", name: "Md. Farhad Hossain", designation: "Assistant Teacher",
    dob: "1975-06-15", mobile: "01721367110", department: "Arts",
    eduLevel: "Higher Secondary", shift: "Day",
    subjects: ["English", "History"], photo: "",
  },
  {
    code: "E000216", name: "Md. Nazrul Islam", designation: "Assistant Teacher",
    dob: "1973-09-11", mobile: "01537633780", department: "Science",
    eduLevel: "Higher Secondary", shift: "Day",
    subjects: ["Physics", "Chemistry", "Biology"], photo: "",
  },
];
...
function IDCard({ teacher, withBg, side }) {
  ...
  if (side === "front") {
    return (
      <div className={`relative w-56 h-36 rounded-xl overflow-hidden shadow-lg flex-shrink-0 select-none
        ${withBg
        ...
        {/* Header stripe */}
        <div className={`px-3 py-1.5 flex items-center gap-1.5 ${withBg ? "bg-white/10" : "bg-blue-600"}`}>
          <div className={`w-4 h-4 rounded-sm flex items-center justify-center text-xs font-bold
            ${withBg ? "bg-white text-blue-700" : "bg-white text-blue-600"}`}>M</div>
          <span className={`text-xs font-semibold leading-tight ${withBg ? "text-white" : "text-white"}`}>
        ...
        <div className="flex gap-2 px-3 pt-2">
          ...
          <div className={`w-12 h-12 rounded-lg flex items-center justify-center flex-shrink-0 text-lg font-bold
            ...
            {teacher.photo
              ? <img src={teacher.photo} alt="" className="w-full h-full object-cover rounded-lg" />
              : initials}
          </div>
          {/* Info */}
          <div className="flex-1 min-w-0">
            <p className={`text-xs font-bold truncate ${withBg ? "text-white" : "text-gray-800 dark:text-gray-100"}`}>
              {teacher.name}
            </p>
            <p className={`text-xs truncate mt-0.5 ${withBg ? "text-blue-100" : "text-gray-500 dark:text-gray-400"}`}>
              {teacher.designation}
            </p>
            <p className={`text-xs mt-0.5 ${withBg ? "text-blue-100" : "text-gray-500 dark:text-gray-400"}`}>
              ID: {teacher.code}
            </p>
            <p className={`text-xs ${withBg ? "text-blue-100" : "text-gray-500 dark:text-gray-400"}`}>
              Dept: {teacher.department}
            </p>
          </div>
        </div>
        {/* Footer */}
        <div className={`absolute bottom-0 left-0 right-0 px-3 py-1 text-xs flex justify-between
  ...
  return (
    <div className={`relative w-56 h-36 rounded-xl overflow-hidden shadow-lg flex-shrink-0 select-none
      ${withBg
        ...
        : "bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 text-gray-800 dark:text-gray-100"}`}>
      <div className={`px-3 py-1.5 text-xs font-bold ${withBg ? "bg-white/10 text-white" : "bg-blue-600 text-white"}`}>
        Teacher Details
      </div>
      <div className="flex gap-3 px-3 pt-2">
        <div className="flex-1 space-y-0.5">
          ...
          <p className={`text-xs ${withBg ? "text-blue-100" : "text-gray-500 dark:text-gray-400"} line-clamp-2`}>
      ...
      <div className={`absolute bottom-0 left-0 right-0 px-3 py-1 text-xs text-center
        ${withBg ? "bg-white/10 text-blue-100" : "bg-gray-50 dark:bg-gray-700 text-gray-400 border-t border-gray-100 dark:border-gray-600"}`}>
        Mohammadpur Kendriya College
      </div>
    </div>
...
export default function TeacherIDCard() {
  const [filters, setFilters] = useState({
    teacherCode: "", printingType: "onlyDownload", formatType: "withBackground",
  });

  const handleChange = (e) => setFilters(f => ({ ...f, [e.target.name]: e.target.value }));

  const filtered = TEACHERS.filter(t => filters.teacherCode === "" || t.code === filters.teacherCode);
  ...
  return (
    <DashboardLayout>
      <div className="space-y-5">
        ...
        <Breadcrumb items={["Dashboard", "Teacher Setup", "Teacher ID Cards"]} />
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
          <div className="flex items-center gap-2.5 px-5 py-4 bg-gray-50 dark:bg-gray-700/30 border-b border-gray-100 dark:border-gray-700">
            ...
            <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">ID Card Settings</span>
          </div>
          <div className="p-5">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="flex flex-col gap-1.5">
                ...
                <select name="teacherCode" value={filters.teacherCode} onChange={handleChange} className={selCls}>
                  <option value="">All Teachers</option>
                  {TEACHERS.map(t => <option key={t.code} value={t.code}>{t.code} — {t.name}</option>)}
                </select>
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Printing Type</label>
                <select name="printingType" value={filters.printingType} onChange={handleChange} className={selCls}>
                  <option value="onlyDownload">Only Download</option>
                  <option value="printAndDownload">Print & Download</option>
                </select>
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Format Type</label>
                <select name="formatType" value={filters.formatType} onChange={handleChange} className={selCls}>
        ...
        {filtered.length === 0 ? (
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm p-12 text-center">
            ...
            <p className="text-sm text-gray-400 dark:text-gray-500">Select a teacher to preview their ID card.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            {filtered.map(teacher => (
              <div key={teacher.code}
                ...
                <div className="px-5 py-3 bg-gray-50 dark:bg-gray-700/30 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold text-gray-700 dark:text-gray-200">{teacher.name}</p>
                    <p className="text-xs text-gray-400 dark:text-gray-500">{teacher.code} · {teacher.designation}</p>
                  </div>
                ...
                {/* Card previews */}
                <div className="p-5">
                  <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-3">Preview</p>
                  <div className="flex flex-wrap gap-4 mb-5">
                    <div>
                      <p className="text-xs text-gray-400 dark:text-gray-500 mb-1.5">Front Side</p>
                      <IDCard teacher={teacher} withBg={withBg} side="front" />
                    </div>
                    <div>
                      <p className="text-xs text-gray-400 dark:text-gray-500 mb-1.5">Back Side</p>
                  ...
                  <div className="flex gap-2 justify-end">
                    ...
                    <button onClick={() => handleDownload(teacher)}
